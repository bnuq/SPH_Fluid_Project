...
public class FluidController : MonoBehaviour
{
    ...
    #region Indirect Draw 관련
        ...
        //수많은 Particles 를 GPU Instancing 을 통해서 그려낼 것이다
        //그에 필요한 arguments 정의

        uint[] argsArray = { 0, 0, 0, 0, 0 };

        ComputeBuffer argsBuffer; //argsArray 를 GPU 에 넘기는 데 필요한 버퍼
        
        Bounds bounds = new Bounds(Vector3.zero, Vector3.one * 0);

        //그리려는 Particle 의 mesh
        ...
        */
        ...
        // 마우스 입력을 확인한다
            ...
            if(Input.GetMouseButtonUp(0))
            {
                ...
                //compute shader 에 두 마우스 지점 간 높이 차이와, 둘 사이 벡터 값을 넘긴다
                shader.SetFloat("yDiff", yRange.y - yRange.x);
    ...
    void InitKernel()
    {
        ...
        /*
        하나의 커널 함수가 사용하는 스레드 그룹 수를 가져온다
        
        나는 마찬가지로 모든 커널 함수가 동일한 스레드 그룹 수를 사용하게 했으며
        x축 방향 스레드만 사용했다
        */
        uint numThreadsX;
        ...
        //각 방향 별 채우고자 하는 Particles 갯수를 이용해서, 그리고자 하는 particle 전체 개수를 구한다
        particleCount = xNum * yNum * zNum;
                
        //전체 Particles 를 다루는 데 필요한 최소 그룹의 수를 구한다
        groupSize = Mathf.CeilToInt((float)particleCount / (float)numThreadsX);
                
        //모든 데이터를 담기 위해 필요한, 배열의 사이즈 정의
    ...
    {
        //기본 particle 정보를 저장하는 배열을 만들고 초기화 한다
        particleArray = new Particle[arraySize];
             
        /* 
            particle 들의 위치 값들을 초기화 해서 담는다
            최대한 particle 들이 정해진 영역 내에서, 일정하게 나열되도록 한다
            그래야 처음에 particle 들이 세팅됐을 때, 지나치게 튕겨나가지 않는다 => 처음에 여기서 실수해서, 다 튕겨져 나감
        */
        for (int i = 0; i < particleCount; i++) { //배열 내 모든 Particles 를 순회
            //해당 Particle 의 위치
            ...
            */
            ...
            //계산 한 위치 값을 가지는 particle 을 배열에 넣는다
            particleArray[i] = new Particle( pos );
        }


        //compute buffer 를 초기화 하고 초기화한 배열 값을 넣는다
        ...
        //argsArray 를 GPU 로 넘기는 버퍼 만들고 배열 값 할당
    ...
    //1-3. Shader 가 필요로 하는 변수 값들을 모두 초기화 한다
    void SetPropertiesOnce()
    {
        ...
        //점성에 의한 힘 계산
            shader.SetFloat("viscosity", viscosity); // 액체의 점성도
        ...
        // 이동 계산
            shader.SetFloat("deltaTime", deltaTime); // 시간 간격, 델타 타임


        //범위 계산
            /*
            움직일 수 있는 범위 값, Particles 가 움직일 수 있는 범위

            입력받은 limitRange 를 사용한다 => particles 를 넣는 개수 이기도 하다
        
            compute shader 에서는 limitRange 를 움직일 수 있는 범위로 사용
            그러니까, 각 영역별로 1 씩 증가시켜서 넘겨 => 생성된 이후 충돌이 일어나지 않도록 하자
            */
        ...
        //Surface Force
            ...
            //특정 값을 넘길 때만, 표면 장력을 계산한다
            shader.SetFloats("surfForceThreshold", surfForceThreshold);

            //특정 particle 이 표면인지 아닌지를 구분할 수 있는 값이 있다
            //surfTrackThreshold 값을 기준으로 해당 Particle 이 표면인지 아닌지를 판단할 수 있다
            material.SetFloat("surfTrackThreshold", surfTrackThreshold);
    }


    
    //1-4. 실행도중 변할 수 있는 값, 변할 때마다 셰이더와 머터리얼에 할당해주어야 한다
    void SetProperties()
    {
        /* Particle */
            //그려지는 particle 의 반지름
    ...
    //Inspector 창에서 값이 변할 때마다, 자동으로 호출되는 함수
    void OnValidate()
    {
        SetProperties();
    }
    ...
    //프로그램 종료 시, 모든 compute buffer 삭제
    void OnDestroy()
    {
